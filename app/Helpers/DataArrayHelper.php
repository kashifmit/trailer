...
class DataArrayHelper {
	...
	public static function trailerTracking($TrailerNo='', $TrailerIds= '', $TrailerUnitNo= '', $groupBy = true)
	{
        ...
        if (!empty($TrailerNo) || !empty($TrailerIds) || !empty($TrailerUnitNo)) {
        	$mapData = SkyBizTrackingModel::select('id','TrailerNo','TrailerUnitNo','Latitude', 'Longitude', 'ClosestLandMark', 'State', 'Country', 'DistanceFromLandmark', 'BatteryStatus', 'Motion_status', 'track_date_time', 'created_at');
     	if (!empty($TrailerNo)) {
